...
def make_summary_df(df, time_unit="month"):
    ...
    for keyword in df["keyword"].unique():
        ...
        summary_rows.append({
            ...
            "시작 검색량": round(start_ratio, 2),
            "최종 기간": format_period_label(end_period, time_unit),
            "최종 검색량": round(end_ratio, 2),
            "평균 검색량": round(avg_ratio, 2),
            "누적 검색량": round(total_ratio, 2),
            "최고 검색량 발생 기간": format_period_label(max_period, time_unit),
            "최고 검색량": round(max_ratio, 2),
            "최저 검색량 발생 기간": format_period_label(min_period, time_unit),
            "최저 검색량": round(min_ratio, 2),
            "증감": round(change, 2),
...
def draw_line_chart(df, title, time_unit="month"):
    ...
    if font_prop:
        ...
        ax.set_xlabel("기간", fontproperties=font_prop)
        ax.set_ylabel("검색량", fontproperties=font_prop)
    else:
        ax.set_title(title)
        ax.set_xlabel("기간")
        ax.set_ylabel("검색량")

    legend = ax.legend(title="키워드", bbox_to_anchor=(1.02, 1), loc="upper left")
    if font_prop and legend is not None:
...
def render_summary_metrics(summary_df, label):
    ...
    top_keyword = summary_df.sort_values("최고 검색량", ascending=False).iloc[0]
    ...
    total_interest_sum = round(summary_df["누적 검색량"].sum(), 2)
    ...
    with col2:
        st.markdown(f"""
        <div class="dashboard-card">
            <div class="card-title">{label} 기간 내 검색량 총합</div>
    ...
    with col3:
        st.markdown(f"""
        <div class="dashboard-card">
            <div class="card-title">{label} 최고 검색량 키워드</div>
            <div class="card-value">{top_keyword["keyword"]}</div>
            <div class="card-sub">{top_keyword["최고 검색량 발생 기간"]}</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col1:
        ...
        <div class="insight-card">
            ...
            <div>
                ...
                <b>{top_keyword["최고 검색량 발생 기간"]}</b>에 최고 검색량를 기록했습니다.
            </div>
        </div>
        """, unsafe_allow_html=True)
...
def render_device_share(summary_pc, summary_mobile):
    ...
    pc_total = float(summary_pc["누적 검색량"].sum())
    mobile_total = float(summary_mobile["누적 검색량"].sum())
    ...
    with c1:
        st.markdown(f"""
        <div class="dashboard-card">
            <div class="card-title">PC 기간 내 검색량 총합</div>
    ...
    with c2:
        st.markdown(f"""
        <div class="dashboard-card">
            <div class="card-title">모바일 기간 내 검색량 총합</div>
...
def render_keyword_power(summary_df, label_kr):
    ...
    power_df = summary_df[["keyword", "누적 검색량", "최고 검색량", "증감률(%)"]].copy()
    power_df = power_df.sort_values("누적 검색량", ascending=False).reset_index(drop=True)
    power_df.index = power_df.index + 1
    power_df["순위"] = power_df.index

    st.markdown(f"#### {label_kr} 키워드 영향력 순위")
    st.dataframe(
        power_df[["순위", "keyword", "누적 검색량", "최고 검색량", "증감률(%)"]],
...
def detect_rising_keywords(df, time_unit="month", min_current=20, min_prev_growth=20, min_avg_growth=30):
    ...
    for keyword in df["keyword"].unique():
        ...
        if (
            ...
            results.append({
                "키워드": keyword,
                "현재 검색량": round(current, 2),
                "직전 구간 검색량": round(previous, 2),
                "최근 3구간 평균 검색량": round(recent_avg, 2),
...
def render_rising_insight(rising_df):
    ...
    st.write(
        ...
        f"급등 확정 시점은 **{top_keyword['급등 확정 시점']}** 입니다. "
        f"현재 검색량은 **{top_keyword['현재 검색량']}** 입니다."
    )
    ...
    try:
        ...
        if rising_point is not None:
            ...
            st.caption(
                ...
                f"검색량: {round(rising_point['ratio'], 2)}"
            )
...
if st.session_state.analysis_run:
    ...
    with main_tab1:
        ...
        compare_df = pd.merge(
            summary_pc[["keyword", "최종 검색량", "증감률(%)", "누적 검색량"]].rename(columns={
                "최종 검색량": "PC 최종 검색량",
                "증감률(%)": "PC 증감률(%)",
                "누적 검색량": "PC 기간 내 검색량 총합"
            }),
            summary_mobile[["keyword", "최종 검색량", "증감률(%)", "누적 검색량"]].rename(columns={
                "최종 검색량": "모바일 최종 검색량",
                "증감률(%)": "모바일 증감률(%)",
                "누적 검색량": "모바일 기간 내 검색량 총합"
            }),
            on="keyword",
            how="outer"
        )
